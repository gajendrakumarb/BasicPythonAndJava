...
public class Ex23NumbersToWords {
    // write your code here
    public static void numberToWords(int number){
        if(number>=0){
            int revNum = reverse(number);
            int orgLength = getDigitCount(number);
            int revLength = getDigitCount(revNum);
            while (revNum>0){
                switch (revNum%10){
    ...
    public static int reverse(int number){
        ...
        String revStringNum = Integer.toString(num);
        StringBuilder newrev = new StringBuilder(revStringNum);
        if(number>0)
        return Integer.parseInt(newrev.reverse().toString());
        else
            return Integer.parseInt("-"+newrev.reverse().toString());
    }
}
